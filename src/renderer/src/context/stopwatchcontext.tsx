...
export function StopwatchProvider({ children }: { children: ReactNode }) {
  ...
  const lap = (defaultNote: string = '') => {
    ...
    const lapTime = lapStartTimeRef.current

    setLaps((prev) => [
      ...prev,
      {
        id: lapCountRef.current,
        timestarted: new Date(lapTime).toLocaleString('en-US', {
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit',
          hour12: true
  ...
  const updateCalendarInLaps = (index: number, calendarId: number, calendarName: string) => {
    setLaps((prev) =>
